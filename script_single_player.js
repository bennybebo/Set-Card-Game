...
function handleClick_single_player(cardNumber) {
    ...
    if (selectedCards.length === 3) {
        checkSelectedCards_single_player();
    }
}


/* SINGLE PLAYER MODE CHECK SELECTED CARDS 
...
* MAKE INCREASE/DECREASE_SCORE_SINGLE_PLAYER
*/
function checkSelectedCards_single_player() {
...
/*
 * SINGLE PLAYER MODE SCORE INCREASE AND DECREASE
 */
 ...
 */
function increaseScore_single_player() {
    score++;
    // display scores for the corresponding player
    document.getElementById("single-player-points").textContent =
        padNumber(score) + " points";
}
/*
   * Decreases current player's score
   * Expects a player as a parameter and returns nothing
   */
function decreaseScore_single_player() {
    score--;
    // display scores for the corresponding player
    document.getElementById("single-player-points").textContent =
        padNumber(score) + " points";
 ...
 */
function resetScores_single_player() {
    let score = 0;
    document.getElementById("single-player-points").textContent = padNumber("0") + " points";
}
...
function newGame_single_player() {
